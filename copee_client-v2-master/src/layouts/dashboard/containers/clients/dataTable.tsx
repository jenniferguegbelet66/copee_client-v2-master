...
export function DataTable<TData, TValue>({
  ...
  const [filter, setFilter] = useState<string>("client_email");
  ...
  const translationTable: TranslationTable = {
    client_id: "id",
    client_date_created: "date de création",
    client_last_name: "nom",
    client_first_name: "prénom",
    client_email: "email",
    client_phone: "téléphone",
    client_fiscal_year_income: "revenu",
    client_birthdate: "date de naissance",
  };
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row, index) => {
                const clientPath = `/dashboard/clients/${row._valuesCache.client_id}`;
                return (
                  <TableRow
                    key={row.id}
                  ...
                  >
                    {row.getVisibleCells().map((cell) => {
                      return cell.id.includes("client_id") &&
                        row._valuesCache.client_id ? (
